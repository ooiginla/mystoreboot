@php
    $orderRequiresTill = ! in_array($order->source, ['offline', 'online'], true);
    $canRecordPayment = ! $orderRequiresTill || ($activeTill && $activeTill->branch_id === $order->branch_id);
@endphp

<dialog class="dialog" id="order-payment-{{ $order->id }}">
    <div class="dialog-header"><div><h2 class="panel-title">Add payment</h2><p class="subtle">{{ $order->order_number }} balance: {{ $currencySymbol }} {{ $money($order->balance_minor) }}</p></div><button class="icon-btn" type="button" data-dialog-close aria-label="Close">x</button></div>
    <div class="dialog-body">
        <form class="mini-form" method="POST" action="{{ route('admin.sales.orders.payments.store', $order) }}">
            @csrf
            @if ($orderRequiresTill && $activeTill)
            ...
            <div class="button-row"><button class="btn secondary" type="button" data-dialog-close>Cancel</button><button class="btn primary" type="submit" @disabled(! $canRecordPayment)>Record payment</button></div>
        </form>
    </div>
</dialog>
