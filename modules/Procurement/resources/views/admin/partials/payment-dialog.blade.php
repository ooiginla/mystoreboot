    ...
    <div class="dialog-body">
        <form class="mini-form" method="POST" action="{{ route('admin.procurement.payments.store') }}">
            ...
            <div class="form-grid">
                ...
                <div class="field"><label>Paid from</label><select name="payment_account_code" required><option value="">Select cash, bank, or payment account</option>@foreach ($paymentSourceAccounts as $account)<option value="{{ $account->code }}">{{ $account->code }} · {{ $account->name }}</option>@endforeach</select></div>
